...
var baseMaterial = new THREE.MeshLambertMaterial({
      ...
      function init() {
        ...
        var errorCallback = function(e) { console.log('Nope!', e); };
        ...
        scene.add( circle );
        circle.update = function(time){ this.rotation.y += .003; }
        å.ready.count++;

        var particles = new THREE.Geometry();
        var pMaterial = new THREE.PointCloudMaterial({
              color: 0xffffff, size: 1, transparent:true, opacity:.25
            });
        for(var i=0;i<500;i++){
          particles.vertices.push(new THREE.Vector3(
            (Math.random() - 0.5) * Math.sin(i)*200,
            (Math.random() - 0.5) * Math.cos(i)*200,
            (Math.random() - 0.5) * Math.sin(i)*200
          ));
        }
        scene.add(new THREE.PointCloud(particles,pMaterial));
        å.ready.count++;
        ...
        book_shelf_l.receiveShadow = book_shelf_l.castShadow = true;
        book_shelf_l.position.y = 10;
        book_shelf_r.receiveShadow = book_shelf_r.castShadow = true;
        ...
        for(var f=0;f<7;f++){
            var sf = new THREE.Mesh(book_shelf_floor_geo,baseMaterial);
            sf.position.set(16, -24 + f * 12, -.5);
            sf.receiveShadow = sf.castShadow = true;
            book_shelf.add(sf);
        }
        å.ready.count++;
        ...
        for(var r=0;r<6;r++){
            for(var b=0;b<12;b++){
                var bk = new THREE.Mesh(book_geo,baseMaterial);
                bk.position.set(5 + b * 2.25, -20 + r * 12, Math.random()*2.0);
                bk.rotation.y = Math.random()*.25;
                bk.rotation.z = Math.random()*.05;
                bk.receiveShadow = bk.castShadow = true;
                book_shelf.add(bk);
            }
        }
...
function clicker(obj){
  ...
  obj.parent.children.forEach(function(el) { explosion = new Explode(el); });
}
...
function Logic(mesh){
    ...
    this.set_specific_morph = function(type,intensity){
        ...
        switch(type){
            case 'default':     mesh.morphTargetInfluences[0] = intensity * speed; break;
            case 'eye_closed':  mesh.morphTargetInfluences[1] = intensity * speed; break;
            case 'laugh':       mesh.morphTargetInfluences[2] = intensity * speed; break;
            case 'mouth_open':  mesh.morphTargetInfluences[3] = intensity * speed; break;
        }
    }
    this.direction = 1;
    this.loop_all_morphs = function(time){
        if(this.counter%50 == 0){ this.direction *= (-1); }
        if(this.counter <= 0){
            this.direction *= (-1);
            curr = (curr < Presets.length) ? curr + 1 : 0;
        }
...
function onDocumentMouseDown( event ) {
    ...
    if ( intersects.length > 0 ){ clicker(intersects[ 0 ].object); }
}
...
function onDocumentMouseMove( event ) {
    ...
    var ray = new THREE.Raycaster( camera.position, vector.sub( camera.position ).normalize() );
    var intersects = ray.intersectObjects( å.targetList );
    if ( intersects.length > 0 ){ mover(intersects[ 0 ].object); }
    if(typeof morph_logic != "undefined"){ morph_logic.loop_all_morphs(time); }
}
